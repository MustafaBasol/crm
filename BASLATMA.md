...
Backend ayakta olmalı (raw smoke scriptleri):
...
Not: `npm run smoke:crm:raw` / `npm run smoke:crm:authz:raw` backend ayakta değilse `curl` connection refused (exit 7) ile düşebilir.
