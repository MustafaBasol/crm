...
export class UniqueSaleNumberPerTenant1762115300000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (driver === 'postgres') {
      await queryRunner.query(
        `ALTER TABLE "sales" DROP CONSTRAINT IF EXISTS "UQ_sales_saleNumber"`,
      );
      await queryRunner.query(
        `ALTER TABLE "sales" DROP CONSTRAINT IF EXISTS "UQ_2e1f8c3a6b0_saleNumber"`,
      );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (driver === 'postgres') {
      await queryRunner.query(
        `ALTER TABLE "sales" DROP CONSTRAINT IF EXISTS "UQ_sales_tenant_saleNumber"`,
      );
      await queryRunner.query(
