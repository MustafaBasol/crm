...
export class RateLimitMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    if (isAuthEndpoint) {
      ...
      res.setHeader('X-RateLimit-Reset', Math.ceil((entry.firstAttempt + this.RATE_LIMIT_WINDOW) / 1000));
    }
  ...
  private isIPAllowed(ip: string): boolean {
    return this.ADMIN_IP_ALLOWLIST.includes(ip) || 
           this.ADMIN_IP_ALLOWLIST.includes('0.0.0.0'); // Allow all if configured
  }
