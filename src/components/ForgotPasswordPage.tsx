      ...
      <div className="flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">{t('auth.forgotPassword', 'Şifremi unuttum')}</h1>
            <p className="text-gray-600 mb-6">{t('auth.forgotPasswordHelp', 'E-postanızı girin, sıfırlama bağlantısı gönderelim.')}</p>

            {sent ? (
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-emerald-900">
                {t('auth.resetEmailSent', 'Eğer bu e-posta kayıtlıysa sıfırlama bağlantısı gönderildi.')}
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <><Send className="h-4 w-4" /> {t('auth.sendResetLink', 'Bağlantı Gönder')}</>}
                </button>
